...
public interface IChannelNoticeService {
    ...
    ChannelRetMsg doNotice(HttpServletRequest request,
                           Object params, PayOrderDO payOrder, MchAppConfigContextResp mchAppConfigContext, NoticeTypeEnum noticeTypeEnum);
    ...
    /** 订单已回调请勿重复回调  (仅异步通知使用) **/
    ResponseEntity doNotifyOrderAlreadyCallBack(HttpServletRequest request);

    /** 订单正在处理中请勿重复回调  (仅异步通知使用) **/
    ResponseEntity doNotifyOrderAlreadyProcessing(HttpServletRequest request);
